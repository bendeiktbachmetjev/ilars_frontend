...
class ApiService {
  ...
  Future<Map<String, dynamic>> validatePatientCode({
    required String patientCode,
  }) async {
    final uri = Uri.parse('$_baseUrl/validatePatientCode');
    try {
      final response = await http.get(uri, headers: _headers(patientCode));
  ...
  Future<Map<String, dynamic>> subscribePatient({
    required String patientCode,
    required String email,
  }) async {
    final uri = Uri.parse('$_baseUrl/subscribePatient');
    final body = jsonEncode({'email': email});
    try {
      final response = await http.post(
        uri, 
        headers: _headers(patientCode),
        body: body,
      );
